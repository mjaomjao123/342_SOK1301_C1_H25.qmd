...
author: "Ioana-Alexandra Bica-Pop"
format: html
editor: visual
---
...
```{r}
df_verdi1000 %>% 
  ggplot(aes(x=tid,y=verdi1000, colour = var))+
  geom_line() +
  geom_point() + 
  labs(
    title = "BNP")
    #suptitle = "Bnp i løpende og faste priser")
#Figuren viser BNP pr person i løpende prisen,rød linje og faste prisen,blå linje 
#Løpende priswe inkluderer effekten av inflasjon, derfor stiger den mye raskere. 
#Inntektene pr person har økt kraftig i løpende priser, særig etter 2000. 
#I faste prisen er 
#veksten mye jevnere og langt mer moderat, fordi prisveksten er trukket fra. 
#Løpende prisen, nominelt BNP pr person viser verdien målt i dagens penger 
#for hvert år. 
#Faste priser, reelt BNP pr person korrigerer for inflasjon, 
#og viser dermed hvor mye inntektene faktisk har kt i realverdi
#Figuren viser BNP per person i både løpende og faste priser mellom 2000 og 2024. Inntektene har økt kraftig i løpende priser, 
#men langt mer moderat i faste priser. Forskjellen skyldes inflasjon: løpende priser 
#måles i årets kroner, 
#mens faste priser korrigerer for prisvekst. 
#For å vurdere faktisk velstandsutvikling og kjøpekraft er faste priser 
#mest relevante,
#mens løpende priser er nyttige
#når man ser på verdier i kroner #for eksempel i budsjettsammenheng.
...
Koden brukte pivot_wider til å omorganisere dataene fra langt til bredt format.

I df_2 ligger orbeservasjonene i langt format med tre kolonner : var (variabelen), tid (år) og verdi (tall). Etter man koder om til df_wide hvor verdiene i var(BNP,MEMOBNP etc) blir egne kolonner, og vi får en rad pr år (tid) På denne måten samles verdiene for hvert år i samme rad, i steden for å være spredt over flere rader.
...
**Merknad:** Det er en del støy i data. Prøv å kombinere geom_point() for å få et bedre inntrykk av den langsiktige utviklingen.

rm(list=ls()) rm(list=ls()) rm(list=ls()) rm(list=ls()) rm(list=ls()) rm(list=ls()) rm(list=ls())

```{r}
# Besvar oppgave IId her
 df_long %>% 
  ggplot(aes(x=tid,y=verdi,color=verdi))+
  geom_point()+
  geom_smooth()+
  labs(
    title = "Prosentvis vekst i nominelt og reelt BNP per person",
  x= "år",
  y= "prosentis vekst",
  color = "variabler"
  ) 
  
#Den blå linjen er resultatet av at vi brute geom_smooth, og punktene viser de årlige observasjonene.
#Vi ser at realveksten enkelte år er negativ da vi ha punkter som går under null på y-aksen. Figuren viser flere år der veksten i reelt BNP pr person er negativ. Eksempler på dette kan være finanskrisen i 2008-2009,oljeprisfall, og koronapandemien i 2020. I slike perioder faller produksjonen, og når BNP ikke vokser raskere enn befolkningen kan BNP pr person bli negativt, slik som vist i denne oppgaven. 
